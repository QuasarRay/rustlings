...
Default Cargo home discovery uses Rust's platform-aware home-directory lookup. On Windows, a Git Bash `HOME` must not override `USERPROFILE`; a Windows regression removes `CARGO_HOME`, supplies a different shell home, and compares real Cargo and direct launches.
