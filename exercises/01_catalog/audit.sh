...
cargo build --manifest-path dev/Cargo.toml --target-dir target --bins --quiet
cargo test --manifest-path dev/Cargo.toml --target-dir target --no-run --quiet
cargo clippy --manifest-path dev/Cargo.toml --target-dir target --all-targets --quiet -- --deny warnings
