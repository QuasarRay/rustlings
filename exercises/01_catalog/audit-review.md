...
The expanded reference suite passes locally on Rust 1.89: strict Clippy, 45 debug unit tests, two added compiled-identity checks, eight original integration tests, and the focused release-profile solution-visibility contract. Seven verifier/process tests check isolation, configuration identity, timeout cleanup preserved-mtime rebuilding, and recovery from a persisted tool failure. The author gate requires 112 independent starter rejections, 112 passing canonical repairs, byte equality for 266 archived files, and 131 compiling negative examples. See [maintainer validation](maintenance.md) for report formats, focused commands, and how to distinguish an escaped fault from an invalid mutant. The current mutation run and latest composed CI result must be checked before merging the stack.
...
## Additional finding from remediation

During overlapping local validation runs, the stronger survey rejected one result as `UNOBSERVED_FAILURE`: mission 47's report contained mission 46's runtime-fault marker. This shows a stale compiled artifact in that run, not a coverage failure in mission 47. It does not establish that ordinary serialized checks reproduce the same interference. The verifier now binds Cargo rebuilds to an input-key environment dependency and checks the compiled identities of both reconstructed crates before grading. A preserved-mtime regression test verifies source changes are observed independently of file timestamp freshness. The observed mismatch was never counted as a caught mutation.

Some overlapping local runs also produced executable permission errors inside Cargo tests. Validation was returned to sequential execution. The grader now recognizes these resource diagnostics and keeps only passing certification reports in its cache; failed Cargo/test reports are retried, so transient infrastructure trouble cannot permanently reject unchanged learner code. An existing saved failure is also ignored.
