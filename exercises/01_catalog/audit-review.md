...
The stronger survey rejected one local run as `UNOBSERVED_FAILURE`: mission 47's report contained mission 46's runtime-fault marker. This is evidence of a stale compiled artifact, not evidence against mission 47's tests. The verifier now binds Cargo rebuilds to an input-key environment dependency and checks the compiled identities of both reconstructed crates before grading. A preserved-mtime regression test verifies source changes are observed independently of file timestamp freshness. The observed mismatch was never counted as a caught mutation.
